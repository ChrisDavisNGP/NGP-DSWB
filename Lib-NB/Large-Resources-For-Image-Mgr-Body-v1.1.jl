    ...
    try
        ...
        for (url in imageDf[:,:url])
            ...
            if (!found)
                ...
                #println("(r\".*",regExpStr)
                println("(r\".*",regExpStr,"\",[\"\",\"\",\"",regExpStr,"\",\"",regExpStr,"\",0,\"\"]),")
#(r".*/adventure/features/everest/first-woman-to-climb-everest-jun",["/adventure/features/everest/first-woman-to-climb-everest-jun"","",",0,""]),
        ...
        for key in keys(urlPatterns)
            value = get(urlPatterns,key,"None")
            if (value[5] > 0)
                println("\"",value[1],"\"",",\"",value[2],"\",\"",value[3],"\",\"",value[4],"\",",value[5],",\"",value[6],"\"")
            end
        end
...
function knownPatterns()
    try
        ...
        urlPatterns = Dict([
            ...
            (r".*/interactive-assets/.*",["Image Mgr(2)","MedHigh",".*/interactive-assets/.*","Interactive Assets",0,""]),
