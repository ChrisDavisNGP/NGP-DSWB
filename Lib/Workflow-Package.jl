...
function pageGroupAnimationWorkflow(TV::TimeVars,UP::UrlParams,SP::ShowParams)
    ...
    #delete!(retailer_results,[:regioncode,:geo_city,:geo_org,:user_agent_major,:operatingsystemversion,:operatingsystemname,:user_agent_model,:referrer])
    delete!(retailer_results,[:regioncode,:geo_city,:geo_org,:user_agent_major,:operatingsystemversion,:operatingsystemname,:user_agent_model])
...
function urlAutoInvWorkflow(TV::TimeVars,UP::UrlParams,SP::ShowParams,CU::CurlParams,NR::NrParams)
    ...
    # Stats on the data Note: 0 and 0 means all data
    statsDF = timeBeaconStats(TV,UP,SP,localTableDF;showAdditional=true,usePercent=true,percentLower=0.0,percentUpper=0.0)
