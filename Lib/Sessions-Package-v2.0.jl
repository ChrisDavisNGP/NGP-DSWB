  ...
  showCriticalPathOnly::Bool=true,showLines::Int64=10)
    try
        ...
        for subdf in groupby(full,[:session_id,:timestamp])
            ...
            if (subdf[1,:timers_t_done] >= TV.timeLowerMs && subdf[1,:timers_t_done] <= TV.timeUpperMs)
                io += 1
                if io <= showLines
                    ...
                    if (reportLevel > 1) println("individualPageReportV2(TV,UP,SP,wellKnownHostDictionary,wellKnownPathDictionary,\"%\",",s3,"\"",s1,"\",",s2,")) #    Time=",s3) end
                end
            end
        end
    catch y
