...
# The Redshift DSN the tables exist on.
dsn = "vfdsn";

# The beacon and resource table names are set here.
...
### The Beacons Used Are Within This Date Range

getBeaconsFirstAndLast()

### This Notebook Covers The Following Domain(s)

startTime = getPeak(getBeaconsFirstAndLast()[symbol("First Beacon")][1], getBeaconsFirstAndLast()[symbol("Last Beacon")][1], :hour)[:hour][1];
endTime = startTime + Dates.Hour(1);
...
chartTreemapResources(startTime, endTime; dimensions = [DEVICE_TYPE]);
...
# Only one dimension can be used at this time to ensure accuracy of data.
deviceType = "Desktop";
chartTreemapResources(startTime, endTime; deviceType = deviceType, dimensions = [RESOURCE_URL], chartTitle = deviceType);
...
# chartLoadTimeStats(startTime, endTime; dimension = DEVICE_TYPE, url = domain, isFirstParty = false, inputFile = file);
chartLoadTimeStats(startTime, endTime; dimension = DEVICE_TYPE, url = domain, isFirstParty = false, outputFile = file);
...
# The url indicates the first-party resource domain to identify the resources against.
# Setting the isFirstParty to false indicates you want to look at all resources that do not come from the domain provided in the url argument.
chartResourceServerStats(startTime, endTime; url = domain, isFirstParty = false);
...
chartLoadTimeDistribution(slowDomain, startTime, endTime)
...
minResourceCount = 30;
chartResources(startTime, endTime; url = slowDomain, minResourceCount = minResourceCount);
