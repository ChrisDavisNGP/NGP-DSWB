...
page_group_table = "$(table)_cluster_view"
...
startTime = DateTime(2016,8,9,9+4,0)
endTime   = DateTime(2016,8,9,17+4,0)
...
query("""create or replace view $(page_group_table) as (select * from $(table) where page_group = '$(productPageGroup)' and "timestamp" between $startTimeMs and $endTimeMs)""")

setTable("$(page_group_table)")
...
results = getLatestResults(hours=2, minutes=0, table_name="$(page_group_table)")

#size(results)
display(results[1:10,:])

#display(results)
localUrl = "%news.nationalgeographic.com/2016/08/drowning-leopard-rescued-well-india%"

#dropped geo_cc
results2 = query("""\
select
"timestamp",page_group, user_agent_device_type,user_agent_family,params_dom_sz,params_dom_ln,params_dom_script,params_dom_img,
timers_t_done,timers_t_resp,timers_t_page
from $(page_group_table)
where
